...
#
# Ported from Matthew Richardson's BrickPiOriginal library for C
# This library can be used in RaspberryPi to communicate with BrickPiOriginal
...
#
# If you #define DEBUG in the program, the BrickPiOriginal.h drivers will print debug messages to the terminal. One common message is
# "BrickPiRx error: -2", in function BrickPiUpdateValues(). This is caused by an error in the communication with one of the
# microcontrollers on the BrickPiOriginal. When this happens, the drivers automatically re-try the communication several times before the
...
# while, and then send 4 more, when it should have just sent 7 at once. During the pause between the packs of bytes, the BrickPiOriginal
...
# Function BrickPiRx() (background function that receives UART messages from the BrickPiOriginal) can return 0 (success), -1 (undefined error that shouldn't have happened, e.g. a filesystem error), -2 (timeout: the RPi didn't receive any UART communication from the BrickPiOriginal within the specified time), -4 (the message was too short to even contain a valid header), -5 (communication checksum error), or -6 (the number of bytes received was less than specified by the length byte).
...
# coded to 7 in the firmware of the BrickPiOriginal. Unfortunately
...
class button:
    ...
    def upd(self,I2C_PORT):
      ...
      self.ljb=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][0]>>1)&1
      self.rjb=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][0]>>2)&1

      #For buttons a,b,c,d
      self.d=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][0]>>4)&1
      self.c=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][0]>>5)&1
      self.b=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][0]>>6)&1
      self.a=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][0]>>7)&1

      #For buttons l1,l2,r1,r2
      self.l2=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1])&1
      self.r2=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>1)&1
      self.l1=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>2)&1
      self.r1=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>3)&1

      #For buttons square,triangle,cross,circle
      self.tri=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>4)&1
      self.cir=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>5)&1
      self.cro=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>6)&1
      self.sqr=~(BrickPiOriginal.SensorI2CIn[I2C_PORT][0][1]>>7)&1

      #Left joystick x and y , -127 to 127
      self.ljx=BrickPiOriginal.SensorI2CIn[I2C_PORT][0][2]-128
      self.ljy=~BrickPiOriginal.SensorI2CIn[I2C_PORT][0][3]+129

      #Right joystick x and y , -127 to 127
      self.rjx=BrickPiOriginal.SensorI2CIn[I2C_PORT][0][4]-128
      self.rjy=~BrickPiOriginal.SensorI2CIn[I2C_PORT][0][5]+129
...
def BrickPiSetTimeout():
    for i in range(2):
        ...
        Array[BYTE_TIMEOUT] = BrickPiOriginal.Timeout&0xFF
        Array[BYTE_TIMEOUT + 1] = (BrickPiOriginal.Timeout / 256     ) & 0xFF
        Array[BYTE_TIMEOUT + 2] = (BrickPiOriginal.Timeout / 65536   ) & 0xFF
        Array[BYTE_TIMEOUT + 3] = (BrickPiOriginal.Timeout / 16777216) & 0xFF
        BrickPiTx(BrickPiOriginal.Address[i], 5, Array)
...
def motorRotateDegree(power,deg,port,sampling_time=.1,delay_when_stopping=.05):
    ...
    for i in range(num_motor):
        BrickPiOriginal.MotorEnable[port[i]] = 1        #Enable the Motors
        power[i]=abs(power[i])
		# Updated for compatibility with Python3
        # BrickPiOriginal.MotorSpeed[port[i]] = power[i] if deg[i]>0 else -power[i]  #For running clockwise and anticlockwise
        # init_val[i]=BrickPiOriginal.Encoder[port[i]]        #Initial reading of the encoder
		#For running clockwise and anticlockwise
        if deg[i]>0:
           BrickPiOriginal.MotorSpeed[port[i]] = power[i]
        elif deg[i]<0:
           BrickPiOriginal.MotorSpeed[port[i]] = -power[i]
        else:
           BrickPiOriginal.MotorSpeed[port[i]] = 0
           init_val[i]=BrickPiOriginal.Encoder[port[i]]        #Initial reading of the encoder     
    ...
    while True:
        result = BrickPiUpdateValues()          #Ask BrickPiOriginal to update values for sensors/motors
        if not result :
            for i in range(num_motor):        #Do for each of the motors
                ...
                if(deg[i]>0 and final_val[i]>init_val[i]) or (deg[i]<0 and final_val[i]<init_val[i]) :
                    ...
                    init_val[i]=BrickPiOriginal.Encoder[port[i]]
                else:
                    run_stat[i]=1
					
					# Updated for compatibility with Python3
                    # BrickPiOriginal.MotorSpeed[port[i]]=-power[i] if deg[i]>0 else power[i]  #Run the motors in reverse direction to stop instantly
                    ...
                    BrickPiOriginal.MotorEnable[port[i]] = 0
                    BrickPiUpdateValues()
...
def BrickPiSetupSensors():
    ...
    for i in range(2):
        ...
        Array[BYTE_SENSOR_1_TYPE] = BrickPiOriginal.SensorType[PORT_1 + i*2 ]
        Array[BYTE_SENSOR_2_TYPE] = BrickPiOriginal.SensorType[PORT_2 + i*2 ]
        for ii in range(2):
            port = i*2 + ii
	    #Jan's US fix###########
            if(Array[BYTE_SENSOR_1_TYPE + ii] == TYPE_SENSOR_ULTRASONIC_CONT):
                Array[BYTE_SENSOR_1_TYPE + ii] = TYPE_SENSOR_I2C
                BrickPiOriginal.SensorI2CSpeed[port] = US_I2C_SPEED
                BrickPiOriginal.SensorI2CDevices[port] = 1
                BrickPiOriginal.SensorSettings[port][US_I2C_IDX] = BIT_I2C_MID | BIT_I2C_SAME
                BrickPiOriginal.SensorI2CAddr[port][US_I2C_IDX] = LEGO_US_I2C_ADDR
                BrickPiOriginal.SensorI2CWrite [port][US_I2C_IDX]    = 1
                BrickPiOriginal.SensorI2CRead  [port][US_I2C_IDX]    = 1
                BrickPiOriginal.SensorI2COut   [port][US_I2C_IDX][0] = LEGO_US_I2C_DATA_REG
		########################
            if(Array[BYTE_SENSOR_1_TYPE + ii] == TYPE_SENSOR_I2C or Array[BYTE_SENSOR_1_TYPE + ii] == TYPE_SENSOR_I2C_9V ):
                AddBits(3,0,8,BrickPiOriginal.SensorI2CSpeed[port])

                if(BrickPiOriginal.SensorI2CDevices[port] > 8):
                    BrickPiOriginal.SensorI2CDevices[port] = 8

                if(BrickPiOriginal.SensorI2CDevices[port] == 0):
                    BrickPiOriginal.SensorI2CDevices[port] = 1

                AddBits(3,0,3, (BrickPiOriginal.SensorI2CDevices[port] - 1))

                for device in range(BrickPiOriginal.SensorI2CDevices[port]):
                    AddBits(3,0,7, (BrickPiOriginal.SensorI2CAddr[port][device] >> 1))
                    AddBits(3,0,2, BrickPiOriginal.SensorSettings[port][device])
                    if(BrickPiOriginal.SensorSettings[port][device] & BIT_I2C_SAME):
                        AddBits(3,0,4, BrickPiOriginal.SensorI2CWrite[port][device])
                        AddBits(3,0,4, BrickPiOriginal.SensorI2CRead[port][device])

                        for out_byte in range(BrickPiOriginal.SensorI2CWrite[port][device]):
                            AddBits(3,0,8, BrickPiOriginal.SensorI2COut[port][device][out_byte])

        tx_bytes = (((Bit_Offset + 7) / 8) + 3) #eq to UART_TX_BYTES
        BrickPiTx(BrickPiOriginal.Address[i], tx_bytes , Array)
...
def BrickPiUpdateValues():
    ...
    while i < 2 :
        ...
        for ii in range(2):
            port = (i * 2) + ii
            if(BrickPiOriginal.EncoderOffset[port]):
                Temp_Value = BrickPiOriginal.EncoderOffset[port]
                AddBits(1,0,1,1)
                Temp_ENC_DIR = 0
                if Temp_Value < 0 :
                    Temp_ENC_DIR = 1
        ...
        for ii in range(2):
            port = (i *2) + ii
            speed = BrickPiOriginal.MotorSpeed[port]
            direc = 0
            if speed<0 :
                direc = 1
                speed *= -1
            if speed>255:
                speed = 255
            AddBits(1,0,10,((((speed & 0xFF) << 2) | (direc << 1) | (BrickPiOriginal.MotorEnable[port] & 0x01)) & 0x3FF))


        for ii in range(2):
            port =  (i * 2) + ii
            #if(BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C_9V):
			#Jan's US Fix##########
            #old# if(BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C_9V):
            if(BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C_9V or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT):
			#######################
                for device in range(BrickPiOriginal.SensorI2CDevices[port]):
                    if not (BrickPiOriginal.SensorSettings[port][device] & BIT_I2C_SAME):
                        AddBits(1,0,4, BrickPiOriginal.SensorI2CWrite[port][device])
                        AddBits(1,0,4, BrickPiOriginal.SensorI2CRead[port][device])
                        for out_byte in range(BrickPiOriginal.SensorI2CWrite[port][device]):
                            AddBits(1,0,8, BrickPiOriginal.SensorI2COut[port][device][out_byte])
                    device += 1
        ...
        if result != -2 :
            BrickPiOriginal.EncoderOffset[(i * 2) + PORT_A] = 0
            BrickPiOriginal.EncoderOffset[(i * 2) + PORT_B] = 0
        ...
        for ii in range(2):
            ...
            if Temp_EncoderVal & 0x01 :
                Temp_EncoderVal /= 2
                BrickPiOriginal.Encoder[ii + i*2] = Temp_EncoderVal*(-1)
            else:
                BrickPiOriginal.Encoder[ii + i*2] = Temp_EncoderVal / 2


        for ii in range(2):
            port = ii + (i * 2)
            #print BrickPiOriginal.SensorType[port]
            if BrickPiOriginal.SensorType[port] == TYPE_SENSOR_TOUCH :
                BrickPiOriginal.Sensor[port] = GetBits(1,0,1)
            #elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_SS :
			#Jan's US fix##########
				#old# elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_SS :
            elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_SS :
			#######################
                BrickPiOriginal.Sensor[port] = GetBits(1,0,8)
            elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_COLOR_FULL:
                BrickPiOriginal.Sensor[port] = GetBits(1,0,3)
                BrickPiOriginal.SensorArray[port][INDEX_BLANK] = GetBits(1,0,10)
                BrickPiOriginal.SensorArray[port][INDEX_RED] = GetBits(1,0,10)
                BrickPiOriginal.SensorArray[port][INDEX_GREEN] = GetBits(1,0,10)
                BrickPiOriginal.SensorArray[port][INDEX_BLUE] = GetBits(1,0,10)
            #elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C_9V :
			#Jan's US fix##########
            #old# elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C_9V :
            elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_I2C_9V or BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT:
            #######################
                BrickPiOriginal.Sensor[port] = GetBits(1,0, BrickPiOriginal.SensorI2CDevices[port])
                for device in range(BrickPiOriginal.SensorI2CDevices[port]):
                    if (BrickPiOriginal.Sensor[port] & ( 0x01 << device)) :
                        for in_byte in range(BrickPiOriginal.SensorI2CRead[port][device]):
                            BrickPiOriginal.SensorI2CIn[port][device][in_byte] = GetBits(1,0,8)
            elif BrickPiOriginal.SensorType[port] in [ TYPE_SENSOR_EV3_COLOR_M3, TYPE_SENSOR_EV3_GYRO_M3 ]:
                BrickPiOriginal.Sensor[port] = GetBits(1,0,32)
            elif BrickPiOriginal.SensorType[port] in [ TYPE_SENSOR_EV3_INFRARED_M2 ]:
                BrickPiOriginal.Sensor[port] = GetBits(1,0,32)
          		###############################################################################################################################################
                # print "Raw returned: "+str(BrickPiOriginal.Sensor[port])
                if 'DEBUG' in globals():
                    if BrickPiOriginal.Sensor[port] > 4278190080:
                        print ("IR SENSOR RETURNED ERROR")
            elif BrickPiOriginal.SensorType[port] in range(TYPE_SENSOR_EV3_US_M0,TYPE_SENSOR_EV3_INFRARED_M5+1):
                BrickPiOriginal.Sensor[port] = GetBits(1,0,16)
            else:   #For all the light, color and raw sensors
                BrickPiOriginal.Sensor[ii + (i * 2)] = GetBits(1,0,10)

            #Jan's US fix##########
            if BrickPiOriginal.SensorType[port] == TYPE_SENSOR_ULTRASONIC_CONT :
                if(BrickPiOriginal.Sensor[port] & ( 0x01 << US_I2C_IDX)) :
                    BrickPiOriginal.Sensor[port] = BrickPiOriginal.SensorI2CIn[port][US_I2C_IDX][0]
                else:
                    BrickPiOriginal.Sensor[port] = -1
			#######################

			#######################
			# EV3 Gyro Mode 0, Adjust sign
            if BrickPiOriginal.SensorType[port] == TYPE_SENSOR_EV3_GYRO_M0 :
                if BrickPiOriginal.Sensor[port] >= 32767:		# Negative number.  This seems to return a 2 byte number.
                    BrickPiOriginal.Sensor[port] = BrickPiOriginal.Sensor[port] - 65535
				# else:					# Positive Number print str(gyro)
			#######################
			# EV3 Gyro Mode 1, Adjust sign
            elif BrickPiOriginal.SensorType[port] == TYPE_SENSOR_EV3_GYRO_M1 :
                # print "Gyro m1!"
                if BrickPiOriginal.Sensor[port] >= 32767:		# Negative number.  This seems to return a 2 byte number.
                    BrickPiOriginal.Sensor[port] = BrickPiOriginal.Sensor[port] - 65535
				# else:					# Positive Number print str(gyro)
